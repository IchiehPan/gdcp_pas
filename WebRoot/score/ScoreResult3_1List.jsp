  ...
  <tr>
    ...
    <table width="98%" border="0" cellspacing="3" cellpadding="0">
      <tr>
        ...
        <td valign="bottom" class="title">未填写的评分考核表_列表</td>
      </tr>
    </table>
    </td>
  </tr>
...
<%int i=1; %>
<c:forEach var="ScoreRuleVO" items="${request.soreRVList}" >
  <tr> 
   <td class="td_02"><%=i%><%i++;%></td>
	<td class="td_02">${ScoreRuleVO.ruleName}</td>
	<td class="td_02" style="color: red;">
	<c:if test="${ScoreRuleVO.status==0}">未填写</c:if>
	<c:if test="${ScoreRuleVO.status==1}">已填写</c:if>
	</td>
	<form name="formn" action="${pageContext.request.contextPath}/NotEvaluateRule_getScoreResultAndSecondIndex.action?scoreRuleId=${ScoreRuleVO.scoreRuleId}" method="post">
	<c:if test="${ScoreRuleVO.status==0}">
		<td class="td_02" align="right">
			<input type=hidden name='ruleName' id="ruleName" value ='${ScoreRuleVO.ruleName}'>
			<input name="button" type="submit" class="buttonface" value="填写考核表" >
		</td>
	</c:if>
	<c:if test="${ScoreRuleVO.status==1}">
		<td class="td_02" align="right">
			<input type=hidden name='ruleName' id="ruleName" value ='${ScoreRuleVO.ruleName}'>
			<input name="button" type="submit" class="buttonface" value="修改或提交" >
		</td>
	</c:if>
	</form>
  </tr> 
