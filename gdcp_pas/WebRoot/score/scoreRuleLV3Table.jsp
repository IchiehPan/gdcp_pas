...
<div id="Title">
    <label for="textfield24" style="font-size: 20;">被考核部门：</label>
    <label id="textfield24" style="font-size: 20;">${requestScope.scoreResultVo.objectName }</label> 
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    <label for="textfield24" style="font-size: 20;">评价主体: </label>
    <label id="textfield24" style="font-size: 20;">${requestScope.scoreResultVo.scorerName }</label> 
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    <label for="textfield24" style="font-size: 20;">日期: </label>
    <c:choose>
      <c:when test="${requestScope.scoreResultVo.status==2 }">
    	<label id="textfield24" style="font-size: 20;">${requestScope.scoreResultVo.commitDate }</label>
      </c:when>
      <c:otherwise>
      	<%
      		SimpleDateFormat format = new SimpleDateFormat("yyyy年MM月dd日");
      		request.setAttribute("nowDate", format.format(System.currentTimeMillis()));
      	%>
   	 	<label id="textfield24" style="font-size: 20;">${requestScope.nowDate }</label>
      </c:otherwise>
    </c:choose> 
</div>
<br>
...
<div id="Table">
    <table width="900" border="1" cellspacing="0" cellpadding="0">
      <tr>
        <td width="88" height="60" class="Biaotou">一级指标</td>
        <td width="88" height="60" class="Biaotou">二级指标</td>
        <td width="478" height="60" class="Biaotou">考核标准等级配分</td>
        <td width="72" class="Biaotou">评分</td>
      </tr>
    	...
    	<c:forEach var="scoreRuleDrtailLV1" items="${requestScope.scoreRuleDrtailLV1s}"  varStatus="status1">
       	...
       	rowspanNum = 0;
       	for(int i=0; i<scoreRuleDrtailLV2s.size(); i++){
       		rowspanNum += scoreRuleDrtailLV2s.get(i).getChilds().size();
       	} 
        %>
		 ...
		 <c:forEach var="scoreRuleDrtailLV2" items="${requestScope.scoreRuleDrtailLV2s}" varStatus="status2">
      	    ...
      	    <%
      	    ArrayList<ScoreRuleDetailVO> scoreRuleDrtailLV3s = ((ArrayList<ScoreRuleDetailVO>)request.getAttribute("scoreRuleDrtailLV2s")).get(lv2Index).getChilds();
            request.setAttribute("scoreRuleDrtailLV3s", scoreRuleDrtailLV3s);
            rowspanNum = scoreRuleDrtailLV3s.size();
      	    %>
     	 	<c:choose>
     		<c:when test="${status2.index>0}">
      	      ...
      	      <tr align="center">
      	      	<td rowspan="<%=rowspanNum %>">
      	      		${scoreRuleDrtailLV2.descRiption}(${scoreRuleDrtailLV2.detailScore }分)
      	      	</td>
      	      	
      	      	<c:forEach var="scoreRuleDrtailLV3" items="${requestScope.scoreRuleDrtailLV3s}" varStatus="status3">
     	 		<input type="hidden" id="LV1Id" name="LV1Id" value="${scoreRuleDrtailLV2.parentId }">
      	      	<input type="hidden" id="LV2Id" name="LV2Id" value="${scoreRuleDrtailLV3.parentId }">
     			<input type="hidden" id="drtailId" name="drtailId" value="${scoreRuleDrtailLV3.scoreDetailId }">

     			<c:choose>
     			 <c:when test="${status3.index>0}">
      	     	  <tr align="center">
      	      	  	<td align="left">${scoreRuleDrtailLV3.descRiption }(0~${scoreRuleDrtailLV3.detailScore }分)</td>
      	      	  	<td>
      	      	  		<label for="textfield20"></label>
      	      	  		<c:if test="${requestScope.scoreResultVo.status==0 }">
      	     	   			<input name="fz" id="textfield20" type="text" class="textfield1" onkeyup="juasNum(this)" onblur="check(this, ${scoreRuleDrtailLV3.detailScore })" onfocus="setNull(this)" >
      	      	  		</c:if>
      	     	  		<c:if test="${requestScope.scoreResultVo.status==1 }">
      	        			<input name="fz" id="textfield20" value="${scoreRuleDrtailLV3.score }" type="text" class="textfield1" onkeyup="juasNum(this)" onblur="check(this, ${scoreRuleDrtailLV3.detailScore })" onfocus="setNull(this)" >
      	     	  		</c:if>
      	     	  		<c:if test="${requestScope.scoreResultVo.status==2 }">
      	     	   			<font  id="textfield20">${scoreRuleDrtailLV3.score }</font>
      	      	    	</c:if>
      	      	   	</td>
      	      	  </tr>
      	    	 </c:when>
      ...
      <tr>
        <td colspan="3" class="Biaotou">合计</td>
