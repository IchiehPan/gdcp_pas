...
public class AllEvaluatedRuleDAOImpl implements AllEvaluatedRuleDAO {
	...
	public List<ScoreResultsVO> getResultList(int scoreRId, int scoreRuleId) throws Exception {
		...
		String querySql = "select  id, OBJECTID,USERNAME,DEPTNAME,SCORERESULT from TB_SCORERESULT , TB_USER ,TB_DEPT "
				+ "where  TB_SCORERESULT.OBJECTID = TB_USER.USERID " + "and TB_DEPT.DEPTID = TB_USER.DEPTID "
				+ "and  SCORERID='" + scoreRId + "'  " + "and  SCORERULEID='" + scoreRuleId + "' "
				+ " order by TB_DEPT.DEPTID";
		RowSet rs = dbAccess.executeQuery(querySql);
		while (rs != null && rs.next()) {
